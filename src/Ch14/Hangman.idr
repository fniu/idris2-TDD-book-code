...
toLetters str = nub (map toUpper (unpack str))
...
data GameCmd : (ty : Type) -> GameState -> (ty -> GameState) -> Type where
  NewGame : (word : String) ->
            GameCmd () NotRunning (const (Running 6 (length (toLetters word))))

  Won :  GameCmd () (Running (S guesses) 0) (const NotRunning)
  Lost : GameCmd () (Running 0 (S ltts)) (const NotRunning)
...
runCmd fuel state (NewGame word) = ok () (InProgress (toUpper word) _ (fromList (toLetters word)))
